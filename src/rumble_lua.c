/*$T rumble_lua.c GC 1.140 04/04/11 15:49:30 */
...
static int rumble_lua_sethook(lua_State *L)
{
	...
	char			svcName[32], svcLocation[32], svcCommand[32];
	rumbleService	*svc = 0;
	cvector			*svchooks = 0;
	int				isFirstCaller = 0;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
static int rumble_lua_deleteaccount(lua_State *L)
{
	...
	else
	{
		...
		if(acc)
		{
			path = strlen(acc->domain->path) ? acc->domain->path : rrdict
				(
					rumble_database_master_handle->_core.conf,
					"storagefolder"
				);
			...
			radb_do
			(
				rumble_database_master_handle->_core.db,
				"DELETE FROM accounts WHERE domain = %s AND user = %s",
				domain,
				user
			);
...
static int rumble_lua_updatedomain(lua_State *L)
{
	...
	if(dmn)
	{
		...
		(
			rumble_database_master_handle->_core.db,
			"UPDATE domains SET domain = %s, storagepath = %s WHERE id = %u",
			newname,
			newpath,
			dmn->id
		);
		...
		(
			rumble_database_master_handle->_core.db,
			"UPDATE accounts SET domain = %s WHERE domain = %s",
			newname,
			dmn->name
		);
		free(dmn->name);
...
{
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	const char	*user, *domain, *password, *arguments;
	const char	*mtype;
	int			x = 0;
	uint32_t	uid = 0;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	if(rumble_domain_exists(domain))
	{
		...
		else if(!x)
		{
			...
			(
				rumble_database_master_handle->_core.db,
				"INSERT INTO ACCOUNTS (user,domain,type,password,arg) VALUES (%s,%s,%s,%s,%s)",
				user,
				domain,
				mtype,
				password,
...
static int rumble_lua_createdomain(lua_State *L)
{
	...
	if(!rumble_domain_exists(domain))
	{
		...
		(
			rumble_database_master_handle->_core.db,
			"INSERT INTO domains (domain,storagepath) VALUES (%s,%s)",
			domain,
			path
		);
		rumble_database_update_domains();
...
static int rumble_lua_accountexists(lua_State *L)
{
	...
	user = lua_tostring(L, 2);
	lua_settop(L, 0);
	if(rumble_account_exists_raw(user, domain))
...
static int rumble_lua_addressexists(lua_State *L)
{
	...
	user = lua_tostring(L, 2);
	lua_settop(L, 0);
	if(rumble_account_exists(0, user, domain))
		lua_pushboolean(L, TRUE);
	else
		lua_pushboolean(L, FALSE);
	return(1);
...
{
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	size_t			workers, busy, idle, sessions, out, in;
	rumbleService	*svc = 0;
	char			capa[1024];
	const char		*svcName;
	c_iterator		iter;
...
static int rumble_lua_config(lua_State *L)
{
	...
	el = lua_tostring(L, 1);
	lua_settop(L, 0);
	if(rhdict(rumble_database_master_handle->_core.conf, el))
		lua_pushstring(L, rrdict(rumble_database_master_handle->_core.conf, el));
	else
		lua_pushnil(L);
	return(1);
}
