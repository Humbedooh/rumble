/*$T comm.c GC 1.140 04/04/11 15:49:29 */
...
ssize_t rumble_comm_printf(sessionHandle *session, const char *d, ...)
{
	...
	if(session->client->tls != NULL)
		len = (session->client->send) (session->client->tls, buffer, strlen(buffer), 0);
	else
		len = send(session->client->socket, buffer, (int) strlen(buffer), 0);
	free(buffer);
	return(len);
}
	...
	for(p = 0; p < 1024; p++)
	{
		...
		if(f > 0)
		{
			if(session->client->recv)
				rc = (session->client->recv) (session->client->tls, &b, 1, 0);
			else
...
char *rumble_comm_read_bytes(sessionHandle *session, int len)
{
	...
	{
		if(session->client->recv)
			rc = (session->client->recv) (session->client->tls, buffer, len, 0);
		else
