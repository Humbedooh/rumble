/*$T smtp_server.c GC 1.140 04/04/11 15:49:31 */
...
void *rumble_smtp_init(void *T)
{
	...
	while(1)
	{
		...
		if(rc == RUMBLE_RETURN_OKAY) rcprintf(sessptr, rumble_smtp_reply_code(220), myName);	/* Hello! */
			...
			printf("Session blocked by module\n");
		...
		if(rc == 421) rumble_comm_send(sessptr, rumble_smtp_reply_code(421422));	/* timeout! */
			...
			rumble_comm_send(sessptr, rumble_smtp_reply_code(221220));				/* bye! */
...
)
{
	...
	if(session->sender)
	{
		...
		rc = rumble_service_schedule_hooks
			(
				(rumbleService *) session->_svc,
				session,
				RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_BEFORE + RUMBLE_CUE_SMTP_MAIL,
				parameters
			);
		...
		rc = rumble_service_schedule_hooks
			(
				(rumbleService *) session->_svc,
				session,
				RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_AFTER + RUMBLE_CUE_SMTP_MAIL,
				parameters
			);
	...
	if(recipient)
	{
		...
		rc = rumble_service_schedule_hooks
			(
				(rumbleService *) session->_svc,
				session,
				RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_BEFORE + RUMBLE_CUE_SMTP_RCPT,
				parameters
			);
		if(rc != RUMBLE_RETURN_OKAY)
		{
...
)
{
	...
	rc = rumble_service_schedule_hooks
		(
			(rumbleService *) session->_svc,
			session,
			RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_BEFORE + RUMBLE_CUE_SMTP_HELO,
			parameters
		);
	if(rc != RUMBLE_RETURN_OKAY) return(rc);
...
)
{
	...
	rc = rumble_service_schedule_hooks
		(
			(rumbleService *) session->_svc,
			session,
			RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_BEFORE + RUMBLE_CUE_SMTP_DATA,
			parameters
		);
	if(rc != RUMBLE_RETURN_OKAY) return(rc);
	...
	(
		log,
		"Received: from %s <%s> by %s (rumble) with ESMTPA id <%s>; %s\r\n",
...
)
{
	...
	rc = rumble_service_schedule_hooks
		(
			(rumbleService *) session->_svc,
			session,
			RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_BEFORE + RUMBLE_CUE_SMTP_RSET,
			parameters
		);
	if(rc != RUMBLE_RETURN_OKAY) return(rc);
	...
	rc = rumble_service_schedule_hooks
		(
			(rumbleService *) session->_svc,
			session,
			RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_AFTER + RUMBLE_CUE_SMTP_HELO,
			parameters
		);
	if(rc != RUMBLE_RETURN_OKAY) return(rc);
	return(250);
	...
	if(sscanf(parameters, "%128[^@\"]@%128[^\"]", user, domain))
	{
		...
		rc = rumble_service_schedule_hooks
			(
				(rumbleService *) session->_svc,
				session,
				RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_BEFORE + RUMBLE_CUE_SMTP_VRFY,
				parameters
			);
		if(rc != RUMBLE_RETURN_OKAY) return(rc);
...
)
{
	...
	rc = rumble_service_schedule_hooks
		(
			(rumbleService *) session->_svc,
			session,
			RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_BEFORE + RUMBLE_CUE_SMTP_NOOP,
			parameters
		);
	if(rc != RUMBLE_RETURN_OKAY) return(rc);
	...
	rc = rumble_service_schedule_hooks
		(
			(rumbleService *) session->_svc,
			session,
			RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_AFTER + RUMBLE_CUE_SMTP_NOOP,
			parameters
		);
	if(rc != RUMBLE_RETURN_OKAY) return(rc);
	return(250);
...
)
{
	...
	rc = rumble_service_schedule_hooks
		(
			(rumbleService *) session->_svc,
			session,
			RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_HOOK_BEFORE + RUMBLE_CUE_SMTP_AUTH,
			parameters
		);
	if(rc != RUMBLE_RETURN_OKAY) return(rc);
	memset(method, 0, 31);
