/*$T mailman.c GC 1.140 04/04/11 15:49:30 */
...
FILE *rumble_letters_open(rumble_mailbox *mbox, rumble_letter *letter)
{
	...
	(
		file,
		"%s/%s.msg",
		strlen(mbox->domain->path) ? mbox->domain->path : rrdict
			(
...
rumble_mailman_shared_bag *rumble_letters_retrieve_shared(uint32_t uid)
{
	...
	dbo = radb_prepare
		(
			...
			"SELECT id, name, subscribed FROM folders WHERE uid = %u",
			uid
		);
	...
	dbo = radb_prepare
		(
			rumble_database_master_handle->_core.db,
			"SELECT id, fid, size, delivered, flags, folder FROM mbox WHERE uid = %u",
			uid
		);
	while((dbr = radb_step(dbo)))
	{
...
uint32_t rumble_mailman_scan_incoming(rumble_mailman_shared_folder *folder)
{
	...
	dbo = radb_prepare
		(
			rumble_database_master_handle->_core.db,
			"SELECT id, fid, size, delivered, flags, folder FROM mbox WHERE folder = %l AND id > %l",
			folder->id,
			folder->lastMessage
		);
...
uint32_t rumble_mailman_commit(accountSession *session, rumble_mailman_shared_folder *folder)
{
	...
	if(!folder) return(0);
	path = strlen(session->account->domain->path) ? session->account->domain->path : rrdict
		(
			rumble_database_master_handle->_core.conf,
			"storagefolder"
		);
	...
	dforeach((rumble_letter *), letter, folder->letters, iter)
	{
		...
		else if(letter->flags != letter->_flags)
		{
			...
			dbo = radb_prepare
				(
					rumble_database_master_handle->_core.db,
					"UPDATE mbox SET flags = %u WHERE id = %l",
					letter->flags,
					letter->id
				);
			radb_step(dbo);
...
{
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	char	*path, *filename, fullname[512];
	size_t	len;
	FILE	*in, *out;
	char	buffer[4096];
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	if(folder)
	{
		path = (char *)
			(
				strlen(account->domain->path) ? account->domain->path : rrdict
					(
						rumble_database_master_handle->_core.conf,
						"storagefolder"
					)
		...
		(
			rumble_database_master_handle->_core.db,
			"INSERT INTO mbox (uid, fid, folder, size, flags) VALUES (%u, %s, %l, %u, %u)",
			account->uid,
			filename,
			folder->id,
			letter->size,
