/*$T imap_server.c GC 1.140 04/04/11 15:49:30 */
...
void *rumble_imap_init(void *T)
{
	...
	ssize_t			rc;
	char			*extra_data, *cmd, *parameters, *line, *tmp;
	const char		*myName;
	int				x = 0;
	time_t			now;
	sessionHandle	*s;
	...
	while(1)
	{
		...
		while(rc != RUMBLE_RETURN_FAILURE)
		{
			...
			if(rc == RUMBLE_RETURN_IGNORE) continue;	/* Skip to next line. */
				...
				rcprintf(&session, "%s BAD Invalid command!\r\n", extra_data);			/* Bad command thing. */
		}
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_login
(
	masterHandle	*master,
	sessionHandle	*session,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_noop
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_capability
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_authenticate
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_select
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
	...
	if(imap->account)
	{
		...
		if(found)
		{
			...
			if(first)
			{
				...
				(
					session,
					"* OK [UNSEEN %"PRIu64 "] Message %"PRIu64 " is the first unseen message.\r\n",
					first,
					first
				);
			}
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_examine
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	if(imap->account)
	{
		...
		if(found)
		{
			...
			{
				exists++;
				if(!first && ((letter->flags & RUMBLE_LETTER_UNREAD) || (letter->flags == RUMBLE_LETTER_RECENT)))
					first = exists;
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_create
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	rumble_args						*args;
	char							*newName;
	rumble_mailman_shared_folder	*folder, *newFolder;
	accountSession					*imap = (accountSession *) session->_svcHandle;
	d_iterator						iter;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	if(args && args->argc == 1)
	{
		...
		rcprintf(session, "%s BAD Invalid CREATE syntax!\r\n", extra_data);
	return(RUMBLE_RETURN_IGNORE);
}
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_delete
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_rename
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	rumble_mailman_shared_folder	*folder, *oldFolder, *newFolder;
	accountSession					*imap = (accountSession *) session->_svcHandle;
	d_iterator						iter;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	if(args && args->argc == 2)
	{
		...
		if(newFolder)
			rcprintf(session, "%s NO RENAME failed: Duplicate folder name.\r\n", extra_data);
		else if(!oldFolder)
		...
		rcprintf(session, "%s BAD Invalid RENAME syntax!\r\n", extra_data);
	return(RUMBLE_RETURN_IGNORE);
}
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_subscribe
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	rumble_args						*args;
	char							*folderName;
	rumble_mailman_shared_folder	*pair, *folder;
	accountSession					*imap = (accountSession *) session->_svcHandle;
	d_iterator						iter;
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	if(args && args->argc == 1)
	{
		...
		rcprintf(session, "%s BAD Invalid SUBSCRIBE syntax!\r\n", extra_data);
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_unsubscribe
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	if(args && args->argc == 1)
	{
		...
		if(!folder)
			rcprintf(session, "%s NO UNSUBSCRIBE failed: No such folder <%s>\r\n", extra_data, folderName);
		else
		{
		...
		rcprintf(session, "%s BAD Invalid UNSUBSCRIBE syntax!\r\n", extra_data);
	rumble_args_free(args);
	return(RUMBLE_RETURN_IGNORE);
}
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_list
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	if(args && args->argc == 2)
	{
		...
		if(!folder)
			...
			rcprintf(session, "* LIST (\\Noselect) \"\" \"%s\"\r\n", folder->name);
		foreach(rmsf, pair, imap->bag->folders, iter)
		{
		...
		rcprintf(session, "%s BAD Invalid LIST syntax!\r\n", extra_data);
	return(RUMBLE_RETURN_IGNORE);
}
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_lsub
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	if(args && args->argc == 2)
	{
		...
		rcprintf(session, "%s BAD Invalid LSUB syntax!\r\n", extra_data);
	return(RUMBLE_RETURN_IGNORE);
}
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_status
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_append
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_check
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_close
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_expunge
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	if(imap->account && (session->flags & rumble_mailman_HAS_SELECT))
	{
		...
		rcprintf(session, "%s NO EXPUNGE: No mailbox selected for expunging!\r\n", extra_data);
	return(RUMBLE_RETURN_IGNORE);
}
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_search
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_fetch
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	rumble_args						*parts;
	rumble_mailman_shared_folder	*folder;
	size_t							a, b, c, d, w_uid, first, last;
	char							line[1024];
	...
	if(sscanf(parameters, "%u:%c[*]", &first, (char *) &last) == 2)
	{
		...
		sscanf(parameters, "%u:%u", &first, &last);
	if(last == 0) last = first;
	if(body)
		sscanf(body, "BODY[%1000[^]]<%u>", line, &octets);
	else if(body_peek)
	...
	foreach((rumble_letter *), letter, folder->letters, iter)
	{
		...
		if(flags)
		{
			...
			(
				session,
				"FLAGS (%s%s%s%s) ",
				(letter->flags == RUMBLE_LETTER_RECENT) ? "\\Recent " : "",
				(letter->flags & RUMBLE_LETTER_READ) ? "\\Seen " : "",
				(letter->flags & RUMBLE_LETTER_DELETED) ? "\\Deleted " : "",
		...
		if(body) letter->flags -= (letter->flags & RUMBLE_LETTER_RECENT);	/* Remove \Recent flag since we're not
																			 * peeking. */
		if(body || body_peek)
		{
			/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_store
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_copy
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
)
{
	...
	printf
	(
		"Copying letters %u through %u (UID = %s) to %lld...\n",
		first,
		last,
		useUID ? "enabled" : "disabled",
		destination->id
	);
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_idle
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
...
/*
 ...
 =======================================================================================================================
 */
ssize_t rumble_server_imap_logout
(
	masterHandle	*master,
	sessionHandle	*session,
	const char		*parameters,
