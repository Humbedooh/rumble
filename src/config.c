/*$T config.c GC 1.140 04/04/11 15:49:29 */
...
/*
 ...
 =======================================================================================================================
 */
static int rumble_compare_value(masterHandle *master, const char *key, const char *oper, const char *value)
{
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	const char	*cval = rrdict(master->_core.conf, key);
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
#define CYGWIN	__CYGWIN__
	...
	if(config)
	{
		...
		char			*key, *value, *buffer, line[512], ckey[129], coper[3], cval[129];
		/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

		for(n = 0; rumble_conf_tags[n].key; n++)
			rsdict(master->_core.conf, rumble_conf_tags[n].key, rumble_conf_tags[n].val);
		memset(ckey, 0, 129);
		memset(cval, 0, 129);
		memset(coper, 0, 3);
			...
			if(!ferror(config))
			{
				...
				if(sscanf(line, "<%511[^ \t>]%*[ \t]%511[^\r\n]>", key, value) >= 1)
				{
					...
					if(!strcmp(key, "if"))
					{
						...
						) if(rumble_compare_value(master, ckey, coper, cval)) ignore &= 0xFFFFFFFE;
					...
					if(!strcmp(key, "else") && !(ignore & 0xFFFFFFFE))
						ignore = (ignore & 0xFFFFFFFE) | (!(ignore & 0x00000001));
				}
