/*$T database.c GC 1.140 04/04/11 15:49:30 */
...
void rumble_database_load_sqlite(masterHandle *master, FILE *runlog)
{
	...
	if(!ftmp)
	{
		...
		(
			dbpath,
			"%s/%s/rumble.sqlite",
			rrdict(master->_core.conf, "execpath"),
			rumble_config_str(master, "datafolder")
		);
		sprintf
		(
			mailpath,
...
uint32_t rumble_account_exists_raw(const char *user, const char *domain)
{
	...
	rc = radb_run
		(
			0,
			"SELECT 1 FROM accounts WHERE domain = %s AND user = %s ORDER BY LENGTH(user) DESC LIMIT 1",
			domain,
			user
		);
	return(rc);
	...
	if(uid)
	{
		dbo = radb_prepare
			(
				...
				"SELECT id, domain, user, password, type, arg FROM accounts WHERE id = %u LIMIT 1",
				uid
			);
	}
	else
...
cvector *rumble_database_accounts_list(const char *domain)
{
	...
	if(rumble_domain_exists(domain))
	{
		dbo = radb_prepare
			(
				rumble_database_master_handle->_core.db,
				"SELECT id, user, password, type, arg FROM accounts WHERE domain = %s",
				domain
			);
		acc = NULL;
