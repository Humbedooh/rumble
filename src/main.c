/*$T main.c GC 1.140 04/04/11 15:49:30 */
...
int rumbleStart(void)
{
	...
	printf("[OK]\n");
	svc = comm_registerService
		(
			master,
			"smtp",
			rumble_smtp_init,
			rumble_config_str(master, "smtpport"),
			RUMBLE_INITIAL_THREADS
		);
	...
	svc = comm_registerService
		(
			...
			"pop3",
			rumble_pop3_init,
			rumble_config_str(master, "pop3port"),
			RUMBLE_INITIAL_THREADS
		);
	...
	svc = comm_registerService
		(
			...
			"imap4",
			rumble_imap_init,
			rumble_config_str(master, "imap4port"),
			RUMBLE_INITIAL_THREADS
		);
...
int main(int argc, char **argv)
{
	...
	if(strlen(r_path))
	{
		...
		sysLog = fopen("rumble_status.log", "a");
	fprintf(sysLog, "\r\n------------------------------------------------------\r\n");
