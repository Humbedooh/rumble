/*$T smtp_worker.c GC 1.140 04/04/11 15:49:31 */
...
void *rumble_worker_process(void *m)
{
	...
	while(1)
	{
		...
		if(rumble_domain_exists(item->recipient->domain))
		{
			printf
			(
				"%s is local domain, looking for user %s@%s\n",
			...
			if(user)
			{
				...
				rc = rumble_server_schedule_hooks(master, (sessionHandle *) item, RUMBLE_HOOK_PARSER);	/* hack, hack,
																										 * hack */
				if(rc == RUMBLE_RETURN_OKAY)
				{
					...
					if(user->type & RUMBLE_MTYPE_MOD)
					{
						...
						rumble_server_schedule_hooks(master, (sessionHandle *) item, RUMBLE_HOOK_FEED); /* hack, hack,
																										 * hack */
						/* done here! */
					}
