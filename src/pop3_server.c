/*$T pop3_server.c GC 1.140 04/04/11 15:49:30 */
	...
	while(1)
	{
		...
		while(rc != RUMBLE_RETURN_FAILURE)
		{
			...
			if(rc == RUMBLE_RETURN_IGNORE) continue;	/* Skip to next line. */
				...
				rumble_comm_send(sessptr, rumble_pop3_reply_code(rc));			/* Bad command thing. */
		}
		...
		if(rc == 421) rumble_comm_send(sessptr, rumble_pop3_reply_code(103));	/* timeout! */
			...
			rumble_comm_send(sessptr, rumble_pop3_reply_code(102));				/* bye! */
...
)
{
	...
	if(found)
		...
		rcsend(session, "-ERR No such letter.\r\n");
	return(RUMBLE_RETURN_IGNORE);
}
