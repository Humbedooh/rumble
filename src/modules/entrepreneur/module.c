...
char* entr_format_page(masterHandle* m, cvector* dict) {
	...
	for (p = strstr(html_template, "[rumble::"); p != NULL; p = strstr(op, "[rumble::") ) {
		...
		if ( !strcmp(key, "date") ) { x = rumble_mtime(); strncpy((char*) (ret + len), x, strlen(x)); len += strlen(x); free(x); }
		else if ( !strcmp(key, "title") ) { x = "Rumble mail server"; strncpy((char*) (ret + len), x, strlen(x)); len += strlen(x); }
		else if ( !strcmp(key, "host") ) { x = (char*) rrdict(m->_core.conf, "servername"); strncpy((char*) (ret + len), x, strlen(x)); len += strlen(x); }
		else if ( !strcmp(key, "version") ) { x = (char*) calloc(1,10); sprintf(x, "%X", RUMBLE_VERSION); strncpy((char*) (ret + len), x, strlen(x)); len += strlen(x); free(x); }
		else {
			if ( rhdict(dict, key) ) {
