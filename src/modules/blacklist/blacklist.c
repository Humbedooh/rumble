/*$T blacklist.c GC 1.140 04/04/11 15:49:31 */
			...
			if(!strcmp(session->sender->domain, badhost))
			{
				...
				if(blacklist_logfile)
				{
					...
					if(fp)
					{
						...
						(
							fp,
							"<blacklist>[%s] %s: Attempt to use %s as sender domain.\r\n",
							ctime(&rawtime),
							session->client->addr,
							badhost
						);
...
#endif
	client = gethostbyaddr
		(
			(char *) &IP,
			(session->client->client_info.ss_family == AF_INET) ? 4 : 16,
			session->client->client_info.ss_family
		);
			...
			if(strstr(addr, badhost))
			{
				...
				if(blacklist_logfile)
				{
					...
					if(fp)
					{
						...
						(
							fp,
							"<blacklist>[%s] %s: %s is blacklisted as a bad host.\r\n",
							ctime(&rawtime),
							session->client->addr,
							addr
						);
		...
		if(session->client->client_info.ss_family == AF_INET)
		{
			...
			while(dnshost)
			{
				...
				if(bl)
				{
					...
					printf
					(
						"<blacklist> %s was blacklisted by %s, closing connection!\n",
						session->client->addr,
						dnshost
					);
#endif
					if(blacklist_logfile)
					{
						...
						if(fp)
						{
							...
							(
								fp,
								"<blacklist>[%s] %s: %s is blacklisted by DNSBL %s.\r\n",
								mtime,
								session->client->addr,
								addr,
								dnshost
...
rumblemodule rumble_module_init(void *master, rumble_module_info *modinfo)
{
	...
	if(blacklist_baddomains->size)
	{
		rumble_hook_function
		(
			master,
			RUMBLE_HOOK_SMTP + RUMBLE_HOOK_COMMAND + RUMBLE_CUE_SMTP_MAIL,
			rumble_blacklist_domains
		);
	}
