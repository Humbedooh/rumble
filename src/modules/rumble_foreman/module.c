 ...
 * Author: Humbedooh
 * 
 * A simple (but efficient) load balancing module for rumble.
...
ssize_t accept_hook(sessionHandle* session) {
    ...
    if ( svc ) {
        ...
        uint32_t workers = cvector_size(svc->threads);  // Number of threads alive
        uint32_t busy = cvector_size(svc->handles);     // Number of threads busy
        uint32_t idle = workers - busy;                 // Number of threads idling
